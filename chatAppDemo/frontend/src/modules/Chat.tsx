...
export const Chat: React.FC = () => {
  ...
  async function classifyuserIntent(nextConversationId: string | undefined, msgs: ChatMessage[]) {
    try {
      ...
      const body: any = nextConversationId
        ? { conversationId: nextConversationId }
        : { messages: msgs.map((m) => ({ role: m.role, content: m.content })) };

      const resp = await fetch(`${BASE_URL}/api/classify-chat`, {
        method: 'POST',
              ...
              if (chunk.done) {
                ...
                setMessages((prev: ChatMessage[]) => {
                  ...
                  if (existingIndex >= 0) {
                    ...
                    copy[existingIndex] = {
                      ...copy[existingIndex],
                      content: copy[existingIndex].content + chunk.delta,
                    };
                    return copy;
                  } else {
                    return [
  ...
  return (
    ...
    >
      ...
      <p style={{ margin: '0 0 1.2rem', fontSize: '0.95rem', color: 'var(--color-text-muted)' }}>
        Ask about destinations, tour styles, seasons or planning phases — I’ll tailor context &
        offers.
      </p>
          ...
          >
            ...
            >
              {messages.map((m) => (
                <div
                  key={m.id}
                  style={{ padding: '4px 0', whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}
                >
        ...
        <aside style={{ flex: '0 0 260px', display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {userAdContent.length === 0 && (
            <div style={{ fontSize: '0.65rem', color: 'var(--color-text-muted)' }}>
              No offers yet. Ask about destinations or trip styles to surface suggestions.
            </div>
          )}
          {userAdContent.map((ad, index) => (
            <div key={index} className="offer-card">
              {ad.imageUrl && (
                <img
                  src={`${BASE_URL}/shared/images/${ad.imageUrl}`}
                  alt={ad.title}
              ...
              <p className="offer-desc">
                {ad.description.length > 140 ? ad.description.slice(0, 137) + '…' : ad.description}
              </p>
            </div>
          ))}
        </aside>

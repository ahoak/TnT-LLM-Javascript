...
app.post('/api/chat', async (req: Request, res: Response) => {
  ...
  // This is a hack for the demo but in reality probably use a vector database or another method to look up information
  ...
  const history = [
    {
      role: 'user',
      content: generateSystemPrompt(userIntentResponse, relatedDocuments, conversations),
    },
  ];
...
// Get records from mock database related to user query- for demo purposes only!
async function classifyDesitinationRecord(
  userMessage: string,
...
// Classify User Intent, Booking Phase, and Tour Type based on chat message
...
// Based on Booking Phase and Tour Type, serve appropriate Ad
// Instead of prompting LLM, we could look this up based on parameters as well
async function getFakePersonalizedContent(
  userMessage: string,
...
// Classify user intent, booking phase, and tourtype explicitly and return classifications and associated Ad
app.post('/api/classify-chat', async (req: Request, res: Response) => {
